    ...
    if (!token) {
      ...
      if (!isAuthorized) {
        ...
        if (!getPlayer) {
          ...
          let isLastPlayer = await getPlayer?.USERS?.filter(
          ...
          if (isLastPlayer.length <= 2) {
            ...
            const minTotalObj = scoreCard.find(
              (data) => data?.USER_ID !== isAuthorized?.ID
            );
            ...
            const winnerId = isLastPlayer?.find(
              (data) => data?.USER_ID !== showPlayerId
            )?.USER_ID;
            ...
            for (let index = 0; index < getPlayer?.USERS?.length; index++) {
              ...
              if (isAuthorized?.ID === USER_ID) {
                ...
                if (currentTurn === USER_ID) {
                  ...
                  const currentIndex = isLastPlayer?.findIndex(
