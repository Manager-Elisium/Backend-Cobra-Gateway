...
async function removePlayerRoomByOwner(io: any, socket: Socket, data: any) {
    try {
        ...
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    const CURRENT_USER = getPlayer.USERS.find((data: any) => (data.USER_ID === USER_ID));
...
export { removePlayerRoomByOwner };